    ...
    if include_numbers == "yes":
        for _ in range(0, math.ceil((len(password)-1) * 0.3)):
            while True:
                ...
                if item.isalpha():
                    if item.islower():
                        password[number_index] = random.randint(0,9)
                        break
    if include_special == "yes":
        for _ in range(0, math.ceil((len(password)-1) * 0.1)):
            while True:
                special_index = random.randint(0, len(password)-1)
                item = str(password[special_index])
                if item.isalpha():
                    if item.islower():
                        password[special_index] = random.choice(special_chars)
                        break
    ...
    while True:
        try:
            ...
            if 8 <= num_of_chars <= 32:
                ...
                print("Please enter a valid range of characters (8-32)")
        except ValueError:
